...
2b. **Actualize the notebook before the handoff.** Run the pass the agent file's § Manager notebook prescribes under handoff discipline — verify each unresolved entry against a live source, retire what died, shrink what survived — and run it yourself rather than spawning a worker, for the reason stated there.{{notebook_hygiene_skill_ref}}
