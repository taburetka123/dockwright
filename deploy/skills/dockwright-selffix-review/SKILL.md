...
3. **Per decision, the bookkeeping is ONE command** (never hand-roll the moves/markers):
   - ACCEPT: first apply the proposal's Diff to its target (verify the target's current state matches the diff context before editing; for `kind: new-asset`, pick the cheapest correct home for the new asset — rule vs skill vs agent-file vs memory — and match the conventions of neighbouring assets). **If the target is cp-deployed** — `setup.sh` copies it into `~/.claude` from the canon (usually at the same relpath, sometimes RENAMED, e.g. `~/.claude/dockwright/presets/X` ← `deploy/presets/X`) — apply the Diff to the CANON SOURCE under `<dockwright_repo>/deploy/` (then commit the dockwright repo + run `setup.sh`), NOT the `~/.claude` copy, which `setup.sh` reverts. A correctly-drafted proposal's `targets:` already points at the canon; if it still points at `~/.claude` for a cp-deployed file, redirect to the canon source before applying. Then:
     `python3 ~/.claude/scripts/gardener_postrun.py decide --proposal <path> --kind accept --reason "<optional note>"`
...
4. **Present a single digest** grouped by recommendation, in this order:
   ...
   **Human-flagged findings sort FIRST.** A finding (or an issue within one) carrying `🚩` / `[MANUAL]` / `**Source**: manual` was deliberately flagged by the user via the `/dockwright-fix` command. Present these in a dedicated `## 🚩 Human-flagged (manual)` group **above** the Apply group, rendered with the 🚩 badge and the flagged text quoted. **Never auto-skip a human-flagged finding** — even a `skip` recommendation is surfaced for the user (the human explicitly asked for attention). Same `###` subsection format; continue the global letter handles.
