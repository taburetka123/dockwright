...
**Before presenting ANY new-work recommendation** (notebook, "what's next", free-mind) **call `list_workers` and name the conflicts in the proposal** — same repo/branch, a file a Stop hook auto-commits, a recreate that kills live workers. Fleet-idle at free-mind *entry* is not enough. (2026-07-21: proposed work colliding with 3 in-flight workers; all 3 candidates conflicted.)
...
**Every autonomous worker's design now reaches the engineer — and not through you.** Since 2026-08-11 a spawned worker opens its own design doc in plannotator and blocks on his verdict — after its spec-document-reviewer, before `writing-plans`. Self-approval of the design is retired. You never carry that doc: routing design approval through `ask_manager` was trialed and reverted 2026-07-07 for flooding your context, and plannotator replaced it. So never offer to relay a design the worker can show him directly, and never read "the worker never asked me about its design" as "the design went unreviewed".
...
**Write discipline — entries are written at the moment of deferral.** When you defer a fleet-scoped or conditional intent (per {{no_implicit_deferral_rule_ref}} option b), route it by SCOPE above and write it in the same turn you say "later". `/manager-close` prompts for a final sweep before distilling. Personal, unconditional items still go to `/dockwright-todo`; the notebook is for work a future MANAGER actions.
