...
10. **Route the MODEL per task at dispatch (model-first, per #9); the spawner default is only the fallback.** Now that the interactive default can be anything, pass an explicit `--model` in `extra_args` keyed to task type so a spawn never silently inherits the user's default:
    - **Trivial/mechanical tasks FIRST — right-size DOWN before applying the defaults below**: when the task carries a complete spec in the prompt, touches ≤2 files, has no design surface and no repo-convention risk, and failure is cheap and obvious (docs edits, config bumps, boilerplate/demo scripts, one-shot text generation) → `--model claude-sonnet-5` and OMIT `--effort` (runtime default). "Trivial" is about the task's judgment surface, not its spec precision — the "detailed spec never argues DOWN" guard later in this principle applies to design-bearing engineering work, not to tasks with no judgment layer at all. (E2E L-12: every task of a fresh install's first fleet — including a 3-line haiku — shipped at the Fable+xhigh impl default; that's the failure this rung exists to stop.)
...
- `NUDGED <name> (limit-reset)` — a banner-scheduled nudge fired: the session-limit banner's reset time ("resets 2:20am (Etc/UTC)") was parsed and the nudge landed at reset+2min, reviving the session right after the limit lifted instead of waiting for the next ladder rung. Parsing is best-effort; the ladder remains the catch-all.
...
On a page: capture the pane (`tmux -L "${DOCKWRIGHT_TMUX_SOCKET:-${CLAUDE_ORCH_TMUX_SOCKET:-dockwright}}" capture-pane -p -t <pane_id>`) and READ THE PENDING COMMAND VERBATIM — never clear a dialog you haven't read. Then:
...
- **Destructive / out-of-scope / credential-touching** (deletes outside the task's tree, force-pushes you didn't sanction, reads of secrets) → do NOT approve. Redirect the worker (`send_manager_to_worker`) or escalate to the human with the verbatim command.
