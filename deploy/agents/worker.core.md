...
2. **Pass your own `claude_sid` to `ask_manager` / `worker_done`.** It is the `session_id` shown in your harness system context — use that value directly. Not visible? It is also exported as `$CLAUDE_CODE_SESSION_ID` (exact name — `$CLAUDE_SESSION_ID` does not exist, so don't `env | grep` around for variants, and don't iterate `~/.claude/dockwright/active/*.json` by hand). Last resort, ONE call: `grep -l "\"name\": \"$CLAUDE_WORKER_NAME\"" ~/.claude/dockwright/active/*.json` — the matching filename minus `.json` is your sid.
