...
- **Worker permissions.** Claude worker spawns default to a deployed headless permission preset: auto permission mode, the dockwright protocol allowlist, local-git verbs (status/diff/log/commit/branch/checkout/fetch/pull/merge/rebase/etc., with shared-`.git` verbs narrowed to non-destructive subcommands), and your configured code roots as additionalDirectories — so a fresh fleet's first spawn doesn't stall on an approval dialog nobody's watching (auto mode lets the safety classifier vet the Bash commands the allowlist can't enumerate, e.g. anything with shell expansion, instead of gating them to a human). `git push`/`git remote`/`git reset` still require approval. Resumes replay the exact settings the session was spawned with, so a resumed worker keeps its original permissions. Opt out via `[spawn] worker_headless_preset = false` (fleet-wide) or a caller-supplied `--settings` (replaces the preset entirely); `extra_args=["--permission-mode", "default"]` re-gates FILE EDITS only for one spawn (the preset's allow rules still auto-approve, since allow rules are consulted before the mode default).
