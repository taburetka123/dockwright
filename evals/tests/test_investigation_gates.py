...
ANSWER = {
    ...
    "required_keywords": ["supplier_to_regions"],
...
GOOD_FINDINGS = (
    ...
    "VALIDATED_CLAIMS: supplier_to_regions has 0 rows [fixtures/schema-dump.txt]\n"
    "the mapper is correct.\n"
)

GOOD_CALLS = [("Read", '{"file_path": "/tmp/work/fixtures/schema-dump.txt"}')]
CORPUS = "SELECT COUNT(*) FROM supplier_to_regions -> 0"
...
def test_missing_required_keyword_fails():
    r = _gate(findings=GOOD_FINDINGS.replace("supplier_to_regions", "sometable"))
